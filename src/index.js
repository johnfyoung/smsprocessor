...
const storeSMS = (sms, bulk) => {
    ...
    sms.$.address = sms.$.address.length === 11 && sms.$.address[0] === "1" ? sms.$.address.substring(1) : sms.$.address;
    sms.$.addresses = [sms.$.address];
    sms.$.msgtype = sms.name;
    bulk.find({ address: sms.$.address, date: sms.$.date }).upsert().replaceOne(sms.$);
}
...
const storeMMS = (mms, bulk) => {
    ...
    mms.$.addresses = mms.addrs.$children.filter(child => typeof child !== "string");
    mms.$.addresses = mms.$.addresses.map(child => {
        let address = child.$.address.replaceAll("+1", "");
        address = address.length === 11 && address[0] === "1" ? address.substring(1) : address;
        return address;
    });
    const parts = mms.parts.$children.filter(child => typeof child !== "string");

    mms.$.body = parts.reduce((body, item) => (item.$.ct === 'text/plain' ? body + item.$.text : body), "");
    mms.$.msgtype = mms.name;
    bulk.find({ address: mms.$.address, date: mms.$.date }).upsert().replaceOne(mms.$);
}
    ...
    if (args[0]) {
        try {
            ...
            await parseStream(args[0], bulk, async () => {
                ...
                console.log("Exectued time (minutes): ", (end_time - start_time) / 1000 / 60);
